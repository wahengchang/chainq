...
Information-oriented. The complete surface of the `chain` CLI and the flow YAML.
...
|---|---|
| `chain init [dir]` | Scaffold a new project: `flow.yaml` (with a `claude -p` profile) + `.gitignore` + `input.txt`. Refuses to overwrite an existing `flow.yaml`. |
| `chain new <name>` | Add another flow YAML (a 2-node starter) to the current project. |
| `chain ui [flow.yaml]` | Open the local web editor (binds `127.0.0.1`). With a path, opens straight into that flow. |
| `chain ls [dir]` | List every `.yaml` flow under `dir` (default: current dir). |
| `chain validate <flow.yaml>` | Static pre-run checks only (DAG, cycles, profiles, `{{ }}` wiring). No model call. Exits non-zero on error. |
| `chain run <flow.yaml> [flags]` | Run the chain, reusing cache. See flags below. |
...
Before a run, chain prints a preflight: `plan: N ai call(s) · M reused · K skipped`.
