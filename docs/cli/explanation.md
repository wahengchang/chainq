...
Understanding-oriented. The ideas behind chainq — why it's shaped this way. For
...
chainq optimizes for that loop. Edit a node, and only that node plus its
...
`cmd` nodes with no declared `inputs:` are the exception: chainq can't see what a
...
chainq is a tool for running **real** local CLI models. Earlier versions shipped a
...
CLI is the fast path; the UI (`chainq ui`) is the same flows with a canvas and
