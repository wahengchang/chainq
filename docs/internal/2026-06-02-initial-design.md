...
> **Historical snapshot — records the plan, not shipped behavior.** Current: the command
> is `chainq`, there is no `fake`/offline profile, and `chainq run` re-runs everything by
> default (`--cache` reuses). See [README](../../README.md) and [current documentation](../README.md).
