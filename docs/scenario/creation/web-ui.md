...
- 中間是**節點畫布**:每個節點一張卡片,滑鼠移上去會出現右緣的接線圓點與單節點執行鈕。
  連線分兩種、一眼可辨:**資料流線**(暖橘實線,`{{ $json }}` 主輸入)與**引用線**
  (冷色虛線,prompt 用 `{{ $('id') }}` / `{{ $node["id"] }}` 跨步取值,可跨好幾層、
  不必接進 `from`)。zoom 工具列有「引用線」開關,可隱藏引用線讓畫面變乾淨。
- 畫布下方一排:**型別下拉** + **+ add step**。
- 節點面板的 **earlier outputs**(上游祖先,不在 `from` 裡):點一下會把
  `{{ $node["id"] }}` 引用插進 prompt,**不會改動 `from`**——這是純跨步取值,
  和畫布上拖線(改 `from`)是兩回事。
