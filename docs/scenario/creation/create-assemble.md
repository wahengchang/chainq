...
對應 n8n 的「Set」這類整理動作:把多條上游貼標籤併成一份、或抽欄位重組。
它也是**合併兩條上游**(fan-in)的工具——讓它吃 `from: [a, b]`,在 prompt 裡分別引用兩邊即可。
每筆輸入跑一次(per-item)。
...
下游只要 `from: combine` 一條,不必知道上面分了幾岔。
...
curl -s http://127.0.0.1:<PORT>/api/add-node \
  ...
  -d '{"path":"<FLOW>","id":"combine","type":"assemble"}'

# 2) 接兩條上游(順序有意義:第一個是 $json)
curl -s http://127.0.0.1:<PORT>/api/connect \
  -H 'content-type: application/json' \
  -d '{"path":"<FLOW>","node":"combine","from":["node1","node2"]}'

# 3) 設組裝範本
curl -s http://127.0.0.1:<PORT>/api/set \
  -H 'content-type: application/json' \
  -d '{"path":"<FLOW>","node":"combine","field":"prompt","value":"【美食】\n{{ $node[\"node1\"] }}\n\n【景點】\n{{ $node[\"node2\"] }}"}'
```
